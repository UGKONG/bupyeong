<template>
  <li :class="{ cut: dateCutYN, yet: dateYetYN }">
    ...
    <p class="dateCutYN" v-if="dateCutYN">마감</p>
    <p class="dateYetYN" v-if="dateYetYN">진행전</p>
  </li>
</template>
...
export default {
  ...
  computed: {
    surveyTitle() {
      return this.idxNumber + ". " + this.item.surveyTitle;
    },
    dateRange() {
      return (
        this.item.startDT.split(" ")[0] + " ~ " + this.item.endDT.split(" ")[0]
      );
    },
    answerViewYN() {
