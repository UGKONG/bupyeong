<template>
  <section class="sModal">
    <header>
      <article>
        <h1>
          ...
          <span>공모사업 신청자 리스트 관리</span>
        </h1>
      </article>
      <article>
        <button @click="$emit('resiBsnsRequestModalClose')">
    ...
    <article class="contents">
      <h2>
        사업 제목 : {{ selectResiBsnsData.title }}
      </h2>
      <table class="ui celled table">
        <thead>
          <tr>
            ...
            <th colspan="2" v-text="tableTitle[3]" />
          </tr>
        </thead>
        <tbody>
          <tr v-for="(req, idx) in requestList" :key="req.requestSeq">
            <td v-text="idx + 1" />
            <td v-text="req.requestName" />
    ...
    <resiBsnsRequestDocViewModal
      v-if="resiBsnsRequestDocViewModalYN"
      :requestData="requestData"
      @resiBsnsRequestDocViewModalOpen="resiBsnsRequestDocViewModalOpen"
      @resiBsnsRequestDocViewModalClose="resiBsnsRequestDocViewModalClose"
    />
  </section>
</template>
...
export default {
  components: {
    resiBsnsRequestDocViewModal: () => import('./resiBsnsRequestDocViewModal.vue')
  },
  props: {
    modalType: String,
    selectResiBsnsData: Object,
    resiBsnsRequestNoModalYN: Boolean,
  },
  data() {
    return {
      editIcon,
      tableTitle: ['신청자명', '연락처', '신청내역', '상태'],
      // 과정 데이터 불러오기
      requestList: [],
      requestData: {},
      resiBsnsRequestDocViewModalYN: false,
    }
  },
  created () {
    useRest({
      url: Store.globalURL + 'admin_resiBsnsRequestList',
  ...
  methods: {
    requestYes(e) {
      ...
      Store.setAlertInfo({
        icon: 'success',
        title: '과정 신청자',
        text: seqObj.requestName + '님이 수락되었습니다.'
      });
    },
    requestNo(e) {
      let seq = e.target.getAttribute('data-seq');
      if (this.resiBsnsRequestNoModalYN) {
        Store.setAlertYN(true);
        Store.setAlertInfo({
          icon: 'warn',
          title: '거절사유 입력',
      ...
      this.$emit('resiBsnsRequestNoModalOpen', seq);
    },
    requestStateView(state) {
      return state == '1' ? '수락됨' : state == '2' ? '거절됨' : '신청중';
    },
    resiBsnsRequestDocViewModalOpen() {
      this.resiBsnsRequestDocViewModalYN = true;
    },
    resiBsnsRequestDocViewModalClose() {
      this.requestData = {};
      this.resiBsnsRequestDocViewModalYN = false;
    },
    requestDocView(e) {
      let seq = e.target.getAttribute('data-seq');
      this.requestData = this.requestList.find(x => x.requestSeq == seq);
      this.resiBsnsRequestDocViewModalOpen();
    }
  },
}
</script>
